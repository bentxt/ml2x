...
- Source evaluation order is strictly left-to-right: operator operands,
  call arguments, list/tuple/record elements, cons operands, and string
  concatenation operands evaluate in source order (short-circuiting of
  `&&`/`||` still skips the right operand; the right operand of `&&`/`||`
  never runs before the left).
- file basename `Demo.mlj` -> `Demo.java`: `public final class Demo { ... }`.
- immutable record -> Java `record name(long x, ...) {}` (nested static).
  Any `mutable` field -> `final class name` with public fields, single ctor;
  mutable fields public non-final, others public final.
- variant -> `sealed interface name<T> {}` plus one nested `record` per ctor
  `implements name<T>`; payload components named v0, v1, ... One-payload ctors
  `Circle of int` -> `record Circle(long v0) implements shape {}`.
  `match` lowers to an if/else `instanceof` chain with pattern variables.
- option -> erased to nullable; `None` = `null` in Java; `Some x` = `x`.
  Match `Some x -> ... | None -> ...` lowers to `v == null ? ... : ...` shape.
  Documented profile rule.  A nested option (`t option option`) cannot be
  erased: `Some None` and `None` would both be `null`.  The payload of a
  `Some` whose element type is itself an option is therefore wrapped in a
  generated `_SomeBox<T>` record, so the two values stay distinct.
- `&&` and `||` always preserve short-circuit evaluation.  When an operand
  carries statements (assignment, `if`, `match`, `let`, `;`), the right
  operand is lowered into an `if` block that runs only when the left value
  demands it; a statement-bearing `while` condition is re-evaluated on every
  iteration (`while (true) { ...; if (!cond) break; body }`).
- match exhaustiveness: only an UNGUARDED arm can prove coverage.  A guarded
  arm may fail at runtime, so a match whose cases are all guarded is
  non-exhaustive.  Every emitted match chain ends in a defensive
  `throw new IllegalStateException(...)` so a checker hole surfaces as an
  exception, never as an invented value.
- tuple -> nested `record Tuple2<A,B>(A v0, B v1){}` / `Tuple3`, emitted once,
  only when used.
- list -> `java.util.List<T>`; literals -> `List.of(...)`; `::` -> helper;
  `[]`/`x::xs` match -> `isEmpty`/`get(0)`/`subList(1, size())`.
  `for x in xs` -> enhanced for.
- int literals stay plain (`5` widens to `long`); string concat `^` -> `+`.
- equality: `=` on int/bool/char/float (primitive operands) -> `==` / `!=`;
  on string/record/variant/list/option/tuple/user objects -> `.equals`.
  The checker resolves operand types; emitter uses the checker type table.
- class -> nested `static final class cname { ... }`: ctor params are
  `private final` fields + a single ctor; `val` fields `private` (final unless
  mutable); methods map to methods; `method private` -> `private`;
  `method static` -> `static`; `inherit itf` -> `implements itf` and all
  interface methods must exist (checker error otherwise).
  When two inherited class types declare the same method name, IDENTICAL
  signatures are allowed and share the class's single implementation;
  incompatible signatures are a checker error ("incompatible signatures").
- a static method has no instance: `self` and bare field/ctor-param names are
  checker errors inside it, so `this` never appears in a static context.
- an interface method must be implemented by a PUBLIC, NON-STATIC method with
  a compatible signature; private or static implementations are checker
  errors (javac would reject them).
- bare field/ctor-param name inside a method -> `this.name`.
- class type -> nested `interface ctname { ... }` (methods no bodies).
- string and char literals are escaped for Java: `"`, `\`, `\n`, `\t`, `\r`
  use their backslash forms; every other character below 0x20 and 0x7F is
  emitted as `\uXXXX`; all other Unicode passes through verbatim as UTF-8.
  Generated files compile with `javac -encoding UTF-8` and produce no
  warnings under `-Xlint:all -Werror`.
- `let main () : unit` -> `public static void main(String[] args)`.
- other top-level functions -> `public static` methods; top-level values ->
  `public static final` fields.  A value whose initializer carries statements
  (`if`, `match`, `let`, `;`, loops) is assigned inside a `static { }`
  initializer block, in declaration order, instead of an illegal class-body
  statement.
