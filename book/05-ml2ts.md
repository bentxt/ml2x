...
Current tests: 11 shared positive fixtures (all but `Lit`, whose 63-bit
int boundary values cannot be represented in JS) — `Core`, `Misc`,
`Objects`, `Interp`, `Edge`, `Order`, `DupItf`, `NewUnit`,
`GuardChain`, `MixedRec`, `GenPoll` — each compiled to TS, type-checked,
run, and diffed against the shared `.out`; 12 shared examples
